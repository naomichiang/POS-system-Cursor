...
> 實體平板（iPad／Android Tablet）餐飲點餐與結帳系統前端（Vue 3 + Vite + Pinia；依賴與指令見 [`package.json`](package.json)）。
...
| [POS-api.md](docs/POS-api.md) | API 需求端點、現有 REST（`createOrder`）、**§7 帳單調整與暫存**（GET 載入／PUT 暫存草案、`lineId`、409）— 見該檔「### 7.」小節 |
...
- **規格書（Google Docs）**：https://docs.google.com/document/d/17BZXlPkMqwTK0y2L7pBIOgRGMTnK3qkWDKH4od21xoI/edit?usp=sharing
