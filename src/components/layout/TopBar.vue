...
<template>
  <div
    class="flex justify-between items-center bg-layer-dark-primary shadow-lg h-16 overflow-hidden border-b border-border-dark-primary">

    <div class="flex items-center h-full">
      <div class="flex justify-center items-center w-[180px] h-full bg-black/50 shrink-0">
        <!-- h-full 讓圖片嘗試填滿高度，max-h-[80%] 防止圖片貼邊，保持呼吸感 -->
        <img :src="logoImage" alt="Logo" class="w-auto h-full max-h-[80%] object-contain" />
      </div>
      <div class="w-2 h-full"></div>
      <div class="flex items-center h-full">
        <button v-for="(item, index) in quickActions" :key="index"
    ...
    <div class="flex items-center h-full">
      ...
      <button
        class="flex justify-center items-center w-16 h-full bg-layer-dark-secondary hover:bg-layer-dark-tertiary active:bg-layer-dark-primary transition-colors">
