...
<template>
  <!-- 側邊滑出容器 -->
  <div
    class="relative flex h-full shrink-0 flex-col transition-all duration-300 ease-in-out"
  ...
  >
    ...
    >
      ...
      <div class="flex-1 w-full overflow-y-auto">
        <div v-if="props.items.length === 0" class="flex h-full items-start justify-center text-text-placeholder text-lg">
          ...
          >
            ...
            <div class="flex-1 min-w-0 flex flex-col">
              ...
              <div class="flex items-start justify-between w-full">
                <div class="flex-1 text-left font-noto text-xl font-normal leading-7 text-text-primary line-clamp-2 break-all">
                {{ item.name }}
                </div>
              ...
              <div
                v-if="isExpanded"
                class="flex items-end justify-between w-full gap-2"
              >
                ...
                <div class="flex-none flex items-center justify-end gap-1 transform translate-y-[7px]">
                  <div
                    class="text-right font-inter text-xl font-medium text-text-helper"
                  >
                    {{ item.price }}
                  </div>
                  <DollarSign
      ...
      <div class="flex h-14 w-full items-stretch shadow-[0px_-4px_8px_0px] shadow-ash-400/40 bg-button-highlight">

        <button
          type="button"
          class="flex flex-none w-btn-sm flex-col items-center justify-center border-r border-border-primary bg-button-highlight-red
          active:scale-95 active:bg-button-highlight-red-hover transition-all duration-200"
          @click="handleDeleteAll"
        >
          <Icon
            icon="tabler:trash-x-filled"
            class="text-ash-700 h-icon-lg w-icon-lg"
          />
        </button>
        ...
        <button
          type="button"
          class="flex flex-1 items-center justify-center text-ash-600 active:bg-button-highlight-hover transition-colors"
        >
      ...
      <div
        class="flex h-16 items-center justify-between self-stretch border-y border-border-primary bg-layer-primary px-4"
      >
        <!-- summary: 已點餐點數 -->
        <div class="flex items-center gap-2">
          <Utensils
            :stroke-width="2.8"
            :class="'text-ash-400 w-6 h-6'"
          />
      ...
      <!-- footer buttons -->
      <div class="flex h-24 w-full items-start justify-start gap-3 p-3">
        <button
          type="button"
        ...
        >
          <div class="text-center font-noto text-2xl font-medium tracking-wide text-white">
            暫存
          </div>
        </button>
        ...
        >
          <div class="flex items-center justify-start pt-1" />
            <div class="justify-start text-center font-noto text-2xl font-medium leading-9 tracking-wide text-white">
              送出
            </div>
          <div class="flex items-center justify-start pt-1" />
        </button>
