...
const props = defineProps({
  /** 桌號，可由父層傳入；若未傳則從 route.params.tableId 取得 */
  tableId: {
    type: String,
    default: undefined,
  },
...
const tableId = computed(() => props.tableId ?? route.params.tableId)

// 訂單顯示資訊（由 store getter 統一提供）
const order = computed(() => orderStore.orderInfoForDisplay)

onMounted(() => {
  orderStore.fetchOrderData(tableId.value)
})

const emit = defineEmits(['remove-payment'])

// 計算總計金額（從 order computed 取得）
...
// 狀態顯示文字（由 orderInfoForDisplay 提供 statusLabel）
      ...
      <div class="flex flex-col items-start self-stretch shrink-0 overflow-hidden">
        ...
        <div class="flex h-16 items-center self-stretch px-4 border-b border-border-primary bg-layer-primary">
          ...
          <div class="flex justify-end">
            <div class="text-text-amount-positive text-right font-inter text-3xl font-bold leading-tight">{{
        ...
        <div class="flex h-16 items-center self-stretch px-4 border-b border-border-primary bg-layer-highlight-yellow">
          ...
          <div class="flex justify-end">
            <div class="text-text-amount-negative text-right font-inter text-3xl font-bold leading-tight">{{ Math.max(0,
              unpaidAmount).toLocaleString() }}</div>
          </div>
        </div>

        <!-- Change -->
        <div class="flex h-16 items-center self-stretch px-4 border-b border-border-primary"
          ...
          <div class="flex justify-end">
            <div class="text-right font-inter text-3xl font-bold leading-tight"
      ...
      <div
        ...
        <div v-for="(payment, index) in payments" :key="index"
          ...
          <div class="flex justify-end items-center gap-1">
            <div
              class="text-text-amount-positive text-right font-inter text-xl font-bold tracking-[-0.02em] leading-tight">
