...
<template>
  <div
    ...
    <div class="flex items-start flex-1 self-stretch">
      <!-- Input & Numpad -->
      <div class="flex flex-col items-center flex-1 self-stretch border-r border-border-primary overflow-hidden">
        <!-- Input Display -->
        <div
      ...
      <div class="flex flex-col items-start self-stretch bg-layer-tertiary">
        <div class="flex flex-col items-start gap-px flex-1 self-stretch overflow-hidden">
          <button v-for="method in paymentMethods" :key="method.id" @click="handlePayment(method.id)" :class="[
            ...
            <div v-if="method.hasIcon && method.icon"
              class="flex w-icon-md h-icon-md justify-center items-center shrink-0">
              <component :is="method.icon" class="w-icon-md h-icon-md text-text-on-color" />
            </div>

            <!-- Label -->
            <div class="text-text-on-color text-center font-noto text-lg font-medium leading-[120%]">
              {{ method.label }}
            </div>
          </button>
        </div>
    ...
    <BaseModal :open="showModal" :title="modalTitle" :content="''" :show-close="false"
      ...
      :danger-button-text="modalType === 'change' ? '完成找零' : modalType === 'unpaid' ? '返回結帳' : ''" @close="closeModal"
      @secondary="closeModal" @primary="closeModal"
