<script setup lang="ts">
  ...
  // 付款列表狀態
  const payments = ref<Array<{ type: 'cash' | 'credit' | 'mobile' | 'gift', amount: number }>>([
  ...
  // 處理新增付款
  const handleAddPayment = (payment: { type: 'cash' | 'credit' | 'mobile' | 'gift', amount: number }) => {
...
<template>
    <div class="flex flex-col w-full h-screen overflow-hidden bg-ash-100 font-noto">
      <TopBar class="flex-shrink-0 z-10" />
